...
extension VoteWannabeView: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row > 0 && indexPath.row <= Wannabe.current.people.count {
        ...
        if indexPath.row == Wannabe.current.people.count + 1 {
            guard let person = Wannabe.current.people.person(name: User.current.name) else { return UITableViewCell() }
